...
class NfcoreTemplate {
    ...
    public static void checkConfigProvided(workflow, log) {
        if (workflow.profile == 'standard' && workflow.configFiles.size() <= 1) {
            log.warn "[$workflow.manifest.name] You are attempting to run the pipeline without any custom configuration!\n\n" +
                    ...
                    "   (1) Using an existing pipeline profile e.g. `-profile test,docker` or `-profile singularity`\n" +
    ...
    public static void email(workflow, params, summary_params, projectDir, log, multiqc_report=[]) {
        ...
        try {
            if (workflow.success) {
                ...
                if (mqc_report.getClass() == ArrayList && mqc_report.size() >= 1) {
                    if (mqc_report.get(0).exists()) {
                        mqc_report = mqc_report.get(0)
                    }
                }
            }
        } catch (all) {
        ...
        log.info "[$workflow.manifest.name] Pipeline execution summary:\n" + email_txt
    }
    ...
    public static void summary(workflow, params, log) {
        ...
        if (workflow.success) {
            if (workflow.stats.ignoredCount == 0) {
                log.info "-${colors.green}[$workflow.manifest.name]${colors.reset} Pipeline completed successfully"
            } else {
                log.info "-${colors.yellow}[$workflow.manifest.name]${colors.reset} Pipeline completed successfully, but with errored process(es)"
            }
        } else {
            log.info "-${colors.red}[$workflow.manifest.name]${colors.reset} Pipeline completed with errors"
        }
    }
    ...
    //
    public static String attachMultiqcReport(multiqc_report) {
        def mqc_report = multiqc_report.getVal()
        if (mqc_report.getClass() == ArrayList) {
            if (mqc_report.size() >= 1) {
                if (mqc_report.get(0).exists()) {
                    return mqc_report.get(0)
                }
            }
        }
        return null
    ...
    //
    // Construct and send a notification to a web hook
    //
    public static void IM_notification(workflow, params, summary_params, projectDir, log) {
        ...
        def engine = new groovy.text.GStringTemplateEngine()
        // Payload template
        def hf = new File("$projectDir/assets/slackreport.json")
        ...
        if (postRC.equals(200)) {
            log.info "[$workflow.manifest.name] Sent summary message to hook_url"
        } else {
            log.info "[$workflow.manifest.name] ERROR: hook_url returned $postRC"
        }
    }
}
